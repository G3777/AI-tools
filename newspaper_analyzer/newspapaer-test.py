...
def process_paper_info(paper, name, title_count):
    ...
    try:
        for article in paper.articles:
            ...
            # Filter articles published within the last month and with content length >= 30
            one_month_ago = datetime.now() - timedelta(days=30)
            if article.publish_date >= one_month_ago and title_count[article.title] < 2 and len(article.text) >= 30:
